...
//  Per-iteration try-catch: one failed frame never kills the thread.
// =======================================================================
static void capture_thread(
    ...
    while (g_running.load(std::memory_order_acquire)) {
        try {
            ...
            ++consecutive_errors;
            std::fprintf(stderr, "[CaptureThread] Frame error (%d): %s\n",
                         consecutive_errors, e.what());
            if (consecutive_errors > 100) {
                std::fprintf(stderr, "[CaptureThread] Too many consecutive errors, stopping\n");
                g_running.store(false, std::memory_order_release);
                break;
            }
            ...
            ++consecutive_errors;
            std::fprintf(stderr, "[CaptureThread] Unknown frame error (%d)\n",
                         consecutive_errors);
            if (consecutive_errors > 100) {
                g_running.store(false, std::memory_order_release);
                break;
...
//  Per-frame try-catch: a single bad frame is skipped, not fatal.
// =======================================================================
...
static void inference_thread(
    ...
    while (g_running.load(std::memory_order_acquire)) {
        try {
            ...
            ++consecutive_errors;
            std::fprintf(stderr, "[InferenceThread] ONNX error (%d): %s\n",
                         consecutive_errors, e.what());
            if (consecutive_errors > 50) {
                std::fprintf(stderr, "[InferenceThread] Too many ONNX errors, stopping\n");
                g_running.store(false, std::memory_order_release);
                break;
            }
            ...
            ++consecutive_errors;
            std::fprintf(stderr, "[InferenceThread] Frame error (%d): %s\n",
                         consecutive_errors, e.what());
            if (consecutive_errors > 50) {
                g_running.store(false, std::memory_order_release);
                break;
            ...
            ++consecutive_errors;
            std::fprintf(stderr, "[InferenceThread] Unknown error (%d)\n",
                         consecutive_errors);
            if (consecutive_errors > 50) {
                g_running.store(false, std::memory_order_release);
                break;
...
//  Per-tick try-catch: control loop never dies from a single bad tick.
// =======================================================================
static void control_thread(
    ...
    while (g_running.load(std::memory_order_acquire)) {
        try {
            ...
            ++consecutive_errors;
            std::fprintf(stderr, "[ControlThread] Tick error (%d): %s\n",
                         consecutive_errors, e.what());
            if (consecutive_errors > 100) {
                g_running.store(false, std::memory_order_release);
                break;
            ...
            ++consecutive_errors;
            std::fprintf(stderr, "[ControlThread] Unknown tick error (%d)\n",
                         consecutive_errors);
            if (consecutive_errors > 100) {
                g_running.store(false, std::memory_order_release);
                break;
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::printf("  Ultra-Low-Latency Pipeline v2.1\n");
        std::printf("=======================================================\n\n");
        ...
        if (!capture.initialize(cfg.capture_region)) {
            ...
            std::fprintf(stderr, "       Desktop Duplication API may not be available.\n");
            std::fprintf(stderr, "       (Not supported over Remote Desktop / some VMs)\n");
            return 1;
        }
        ...
        if (!detector.initialize(cfg.model_path, cfg.infer_input_size,
            ...
            std::fprintf(stderr, "       Ensure the .onnx model file exists.\n");
            return 1;
        }
